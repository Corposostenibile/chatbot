...
### Script di Gestione Locale

Il progetto include uno script completo per la gestione dello sviluppo locale: `./scripts/local.sh`
...
2. **Sviluppo quotidiano**:
   ```bash
   # Avvia il database (se non già attivo)
   ./scripts/local.sh local-db-start
...
4. **Reset completo** (se necessario):
   ```bash
   ./scripts/local.sh local-db-reset
   ```
